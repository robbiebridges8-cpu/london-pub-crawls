...
export default function CrawlCard({ crawl }: CrawlCardProps) {
  ...
  const pubCount = crawl.pubCount ?? 0;
  const diffStyle = difficultyStyles[crawl.difficulty] || difficultyStyles['Easy'];

  const inner = (
    <div
      className={`bg-[var(--surface)] h-full flex flex-col ${crawl.live ? '' : 'opacity-60'}`}
      style={{ borderTop: `4px solid ${accentColor}` }}
    >
      <div className="p-6 flex flex-col flex-grow">
        {!crawl.live && (
          <span className="font-label text-[10px] uppercase tracking-wider text-[var(--muted)] mb-2">
            Coming Soon
          </span>
        )}
        <h3 className={`font-card text-lg font-bold text-[var(--ink)] mb-2 ${crawl.live ? 'group-hover:text-[var(--claret)]' : ''} transition-colors`}>
          {crawl.name}
        </h3>
  ...
  return (
    <Link href={`/${crawl.slug}`} className="group block transition-all duration-200 hover:shadow-lg">
      {inner}
    </Link>
  );
}
